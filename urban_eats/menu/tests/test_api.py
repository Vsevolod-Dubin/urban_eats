...
def test_create_order_without_auth_returns_401(api_client):
    response = api_client.post("/api/orders/", data={"dishes": [1]}, format="json")
...
def test_can_create_order_with_auth_returns_201(test_user):
    ...
    response = client.post("/api/orders/", data={"dishes": [dish.id]}, format="json")
...
def test_order_with_invalid_dish_id_returns_400(test_user):
    client, _ = test_user
    response = client.post("/api/orders/", data={"dishes": [9999]}, format="json")
